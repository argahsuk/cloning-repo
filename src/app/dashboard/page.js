...
import { IssueCard } from "@/components/IssueCard"; // Path synchronized
...
export default function DashboardPage() {
  ...
  // Maintains your robust "Zero Cards" fix
  const issues = Array.isArray(data) ? data : (data?.issues || []);
  console.log(data)
  const maxUpvotes = issues.length > 0 
    ? Math.max(...issues.map((i) => i.upvoteCount || 0)) 
    : 0;
  ...
  return (
    <>
      ...
      <main className="mx-auto max-w-7xl px-4 py-8">
        {/* Title Section matching the clean look */}
        <div className="mb-8">
        ...
        {/* Filters Bar with updated padding and spacing */}
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-border py-20">
            ...
            <p className="mt-1 text-sm text-muted-foreground">{error?.message || "Unable to load issues"}</p>
          </div>
        ) : issues.length === 0 ? (
          <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-border py-20">
